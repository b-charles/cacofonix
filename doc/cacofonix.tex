...
\begin{itemize}
	...
	\item A feature with a monophonic scope modify the next notes of the monophonic part. If the feature is placed in a monophonic section, which is a part of a polyphonic section, the other monophonic parts (which are played in parallel) are not modified, influenced or perturbed, in any way. If, after the modification, a new polyphonic section starts, the feature will be apply in every new monophonic parts. Last point: after the polyphonic section, the last modification of the last written monophonic part will be active. See this example (see section \ref{sec:PlainDynamics} for explications about dynamics):
\begin{lstlisting}
NoteInit
...
\myself{A feature with a note scope modify a note. When it's a monophonic scope, that modify the monophic section. A channel scope modify the channel, which means the sheet, and a global feature modify all sheets. It's almost logical.}
...
In fact, the \lstinline!:! operator is like the \lstinline!+! operator: it merges two array of notes in the same way. The difference is the optional argument, a string which indicates the accent and if the note is detached. The string is composed with the characters of the table \ref{table:charsOfColon}: only one accent can be specify (\lstinline!'>'!, \lstinline!'.'!, \lstinline!'^'! or \lstinline!'-'!).

The character \lstinline!'|'!, for a detached note, can be added after or before (for more information about detached notes, see the section \ref{sec:TiesAndSlurs}).

The character \lstinline!'$'! indicate an arpeggiated chord. If the note is tied (and not detached), the arpeggio is ignored.

\begin{table}
	\center
\begin{tabular}{ccp{7cm}}
\lstinline!'>'! & accent & \verb!toto! raise the dynamics of the note \\
