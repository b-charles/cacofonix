...
\end{lstlisting}
	\item A feature with a channel scope modify \dots the channel, and so all played monophonic sections of the sheet at the same time. Be careful: remember that percussions share the same channel!
...
Like plain dynamics, \cacofonix look at the current dynamic at the start of the note, and apply this dynamic only for the attack of the note. So you can't play crescendo or decrescendo on one note.
...
\begin{meenv}%
No reason. I guess I was boring. But, wait\dots If sometimes ghost notes are added, it's pretty hazardous to use indexation like \lstinline!mysheet(5:25)!. How to get safely an extract of a sheet?%
\end{meenv}
...
\begin{lilypond}
\relative c'' { \new Staff { \time 4/4 c\sustainOn a b2\sustainOff g4 a b\sustainOn c d\sustainOn g, c2 } }
\end{lilypond}
...
sheet = [ ...
	SustainOn +C:N4 A:N4 B:N2 SustainOff bar ...
	G:N4 A:N4 SustainOn B:N4 +C:N4 bar ...
	SustainOn +D:N4 G:N4 +C:N2 bar ];
\end{lstlisting}
...
Marker can be used everywhere, but it's common to place the marker at the start of a mesure. Markers are very useful when \cacofonix crashs: the error message give the problematic measure in function of the last marker read.

\begin{lilypond}
\new GrandStaff <<
\new Staff { \key g \major \time 2/4 \relative c'' { g4^\markup{ A } a b g b^\markup{ B } c d2 d8.^\markup{ C } e16 d8 c b4 g g^\markup{ D } d g2 } }
\new Staff { \key g \major \time 2/4 \clef bass \relative c' { g2 d' g, d' g, d' g, d' } } >>
\end{lilypond}
\begin{lstlisting}
>> NoteInit
>> markA = Note( 'Marker', 'A' );
...
>> sheet1 = [ ...
	...
	markB B:N4 +C:N4 bar ...
	+D:N2 bar ...
	markC +D:N8' +E:N16 +D:N8 +C:N8 bar ...
	B:N4 G:N4 bar ...
	markD G:N4 D:N4 bar ...
	G:N2 bar ];
>> sheet2 = [ ...
...
>> cacofonix( 'FrereJacques', 'Tempo', 100, ...
	'Piccolo', sheet1, ...
	'Percussive Organ', sheet2 );
??? Error using ==> cacofonix
The marker C does not match an existing ...
		marker (measure 6 (3 after ...
		marker B), sheet 2 (Percussive Organ)).
 
>> fprintf( char( [ 79 117 112 115 33 92 110 92 110 ] ) )
Oups!
...
When \lstinline!^! is used only with a double, \cacofonix search basically the name the most likely. So, in our example, raise \lstinline!scale! of $3$ half steps will make the scale of D flat major (five flats at the key), not the scale of C sharp major (seven sharps at the key):
\begin{lstlisting}
>> scale^3
$|  Db  |  Eb  |  F   |  Gb  |  Ab  |  Bb  |  C   $
\end{lstlisting}
