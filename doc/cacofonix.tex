...
Sometimes, it can be useful to extract a piece of a sheet which has been already transcode: to play something twice or to add a coda\dots This operation can be very complicated with polyphonic sections, so the operator \lstinline!or! (\lstinline!|!) realize neatly this function.
...
>> finalSheet = sheet|myMarker;
\end{lstlisting}
Of course, the marker (or a goto marker) have to be present in the sheet, in every monophonic parts at the right place.
...
So, this is how extract a piece from a marker to an other:
...
\myself{The markers are included in the extracted sheet. If you are using a player which displays the markers in the midi file, you should see them.}
