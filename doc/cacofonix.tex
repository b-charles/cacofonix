...
To start a section of a sheet with a precise dynamic, you just have to add a special note: \lstinline!Note('dynamics',dyn)!, where \lstinline!dyn! is a string found in the table \ref{tab:dynamics}.

Before examples, two precisions: when a note is \dots
\me{ Don't! We are here precisely to give precisions! }
\myself{ Yes, every man to his own trade. }
\me{ Forced to get angry. Incredible. So, what are we talking about? }
\myself{ Well, when a note is played, \cacofonix look at the current dynamic and apply this dynamic for all the duration of the note, so you can't play fun dynamics like \sfz }
\me{ \dots yet. }
