...
The operator \lstinline!times! (\lstinline!.*!) was the first attempt to apply tonalities to a rhythmic (riff). The goal wasn't completely achieved and void notes have been imagined to answer to the problem with more flexibility. But the operator have shown that it is very useful.
...
The algorithm isn't as complicated as it seems. In fact, the steps \stepref[T]{checki} and \stepref[T]{inci} and the steps \stepref[T]{checkj} and \stepref[T]{incj} define two imbricated \lstinline!for! loops: each note of \lstinline!A! is merged with each note of \lstinline!B! (step \stepref[T]{addAiBj}), unless a note can't be merged (steps \stepref[T]{addAi} and \stepref[T]{addBj}).
