...
% ALGORITHM
\newenvironment{algo}[1]{\par \noindent \textbf{#1}}{}
\newcommand{\step}[2]{\par \noindent \textbf{#1} #2}
...
When you use \lstinline!.*! on two sheets, the algorithm is a little more complicated. Says we have two sheets:
\begin{lstlisting}
>> A = [ a1 a2 a3 ... an ];
>> B = [ b1 b2 b3 ... bm ];
\end{lstlisting}
The new sheet, \lstinline!C = A.*B!:
\begin{algo}{Operator \lstinline!.*!}
	\step{1. Initialize:}{Set \lstinline!i = 1! and \lstinline!j = 1!. Initialize \lstinline!C = []!.}
	\step{2. Check \lstinline!A!:}{If \lstinline!i > n!, stop the algortihm.}
	\step{3. Add \lstinline!A(i)!:}{If the \lstinline!i!th element of \lstinline!A! is not a playable note (neither a duration nor a tonality, so a bar, a crescendo\dots) add this element at the end of \lstinline!C!: \lstinline!C(end+1)=A(i)! and go to step 8.}
	\step{4. Check \lstinline!B!:}{If \lstinline!j > m!, sets \lstinline!j = 1! and go to 8.}
	\step{5. Add \lstinline!B(j)!:}{If the \lstinline!j!th element of \lstinline!B! is not a playable note (see step 3), add this element at the end of \lstinline!C! and go to the step 7.}
	\step{6. Add \lstinline!A(i)+B(j)!:}{Merge and add the new note: \lstinline!C(end+1) = A(i)+B(j)!.}
	\step{7. Increase \lstinline!j!:}{Set \lstinline!j = j + 1!. Go to the step 4.}
	\step{8. Increase \lstinline!i!:}{Set \lstinline!i = i + 1!. Go to the step 2.}
\end{algo}
