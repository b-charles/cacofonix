...
\begin{itemize}
	...
	\item A feature with a monophonic scope modify the next notes of the monophonic part. If the feature is placed in a monophonic section, which is a part of a polyphonic section, the other monophonic parts (which are played in parallel) are not modified, influenced or perturbed, in any way. If, after the modification, a new polyphonic section starts, the feature will be apply in every new monophonic parts. Last point: after the polyphonic section, the last modification of the last written monophonic part will be active. This is a good example (see section \ref{sec:PlainDynamics} for explications about dynamics):
\begin{lstlisting}
NoteInit
...
This is an example of the dynamics using: \\
\begin{lilypond}
...
You can indicate a fermata with \lstinline!Note( 'fermata', duration, playedDuration )!: \lstinline!duration! is the duration of the fermata (\emph{on the sheet}) and \lstinline!playedDuration! is wanted duration of the fermata with the current tempo (\emph{played duration}).

\subsubsection{Example}

This is an example of the tempo using: \\
\begin{lilypond}
	\relative c'' { \new Staff { \time 4/4 c2\ff g a\> b c2\mp b2 g1\< a1\sfz } }
\end{lilypond}
