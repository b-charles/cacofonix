...
A void note is a tonality \note, but without defined tonality yet. You can do anything with a void note, like a tonality note: create a chord (you can have only one void note per chord), set a duration, set accents, transpose\dots but the \cacofonix function doesn't accept void notes in sheets -- except for percussion sheets, where \emph{real} tonality \note are not accepted. To set a final tonality to a void note, you have to use the operator \lstinline!*!.
...
\begin{lilypond}
\new Staff {
	\relative c' { \time 4/4 r4^\markup G e4 d8 c8 b4 b4^\markup C c8 b8 ~ b8 a8 g8 a8 b4 c8 b8 ~ b8 a8 g8 a8 \time 2/4 b4^\markup D c4 \time 4/4 b1^\markup E }
