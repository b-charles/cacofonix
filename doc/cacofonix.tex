...
\ForEachX{,}{\\ \noindent \makebox[0pt][r]{\textbf{\titlealgo\the\thislevelcount} }[\csname title@\the\thislevelcount\endcsname] \csname text@\the\thislevelcount\endcsname}{\listofsteps}
}
...
The operator \lstinline!mtimes! (\lstinline!.*!) was the first attempt to apply tonalities to a rhythmic (riff). The goal wasn't completely achieved and void notes have been imagined to answer to the problem with more flexibility. But the operator have shown that it is very useful.
...
\begin{algo}{M}{Operator \lstinline!.*!}{Given two array \lstinline!A! and \lstinline!B!, we compute \lstinline!C = A.*B! with this algorithm:}
