...
\newlength{\titlesteplen}
\newlength{\titletemplen}
\newbox{\titlestepbox}

\makeatletter 

% format a title of a step
\newcommand{\formattitlestep}[2]{Step #1. \textbf{#2:} }
...
% test if the new title is longer than the previous
\sbox{\titlestepbox}{\formattitlestep{\thealgostep}{#2}}%
\settowidth{\titletemplen}{\usebox{\titlestepbox}}%
\ifthenelse{\lengthtest{\titletemplen>\titlesteplen}}{\setlength{\titlesteplen}{\titletemplen}}{}%
}

% ref to a step
\newcommand{\stepref}[1]{\ref{Algo\thealgonum.Step #1}}

% algo env
\newenvironment{algo}[1]{%
\stepcounter{algonum}%
\setlength{\titlesteplen}{0pt}%
\par \noindent \textbf{#1}%
}{%
\ForEachX{,}{\\ \noindent \makebox[\titlesteplen][l]{\formattitlestep{\the\thislevelcount}{\csname title@\the\thislevelcount\endcsname}} \csname text@\the\thislevelcount\endcsname}{\listofsteps}
}
...
The operator \lstinline!.*! was the first attempt to apply tonalities to a rhythmic (riff). The goal wasn't completely achieved and void notes have been imagined to answer to the problem with more flexibility. But the operator have shown that it is very useful.
...
When you use \lstinline!.*! on two sheets, the algorithm is a little more complicated. Says we have two sheets, \lstinline!A! and \lstinline!B!, the next algorithm describes how \lstinline!C = A.*B! is computed:
\begin{algo}{Operator \lstinline!.*!}
	\step{Initialize}{Initialize \lstinline!C = []!. Set \lstinline!i = 1! and \lstinline!j = 1!.}
	...
	\step{Add \lstinline!B(j)!}{If the \lstinline!j!th element of \lstinline!B! is not a playable note (see step \stepref{addAi}), add this element at the end of \lstinline!C! and go to the step \stepref{incj}.}
	\step{Add \lstinline!A(i)+B(j)!}{Merge and add the new note: \lstinline!C(end+1) = A(i)+B(j)!.}
	\step[incj]{Increase \lstinline!j!}{Set \lstinline!j = j + 1!. Go to the step \stepref{checkj}.}
	\step[inci]{Increase \lstinline!i!}{Set \lstinline!i = i + 1!. Go to the step \stepref{checki}.}
\end{algo}
